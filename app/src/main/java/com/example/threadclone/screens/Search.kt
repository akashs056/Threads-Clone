...
fun Search(navHostController: NavHostController){
    ...
    val threadAndUser by searchViewModel.Users.observeAsState()
    ...
    Column {
        ...
        LazyColumn {
            if (threadAndUser!=null && threadAndUser!!.isNotEmpty()) {
                val filterItems =
                    threadAndUser?.filter { it.name.contains(search, ignoreCase = true) }
                items(filterItems ?: emptyList()) { pairs ->
                    UserItem(user = pairs, navHostController = navHostController)
                }
            }
        }
    }
