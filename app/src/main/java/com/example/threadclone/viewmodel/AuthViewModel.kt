...
class AuthViewModel : ViewModel() {
    ...
    private fun getData(uid: String,context: Context) {
        userRef.child(uid).addListenerForSingleValueEvent(object :ValueEventListener{
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                SharedPref.storeData(user!!.name,user.email,user.bio,uid,user.userName,user.imageUrl,context)
            }
    ...
    private fun saveData(
        ...
        val user=UserModel(email,password,name,bio,userName,imageUrl,uid)
        userRef.child(uid!!).setValue(user).addOnSuccessListener {
            SharedPref.storeData(name,email,bio,uid,userName,imageUrl, context)
